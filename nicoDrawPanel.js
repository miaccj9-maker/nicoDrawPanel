/* Qixian Draw Panel (Nico Draw Panel) - SillyTavern Extension v1.10.0
   ...
   v1.8.0 进度条 + 弹幕歌词；v1.9.0 导航编辑 + 快拍替换。
   v1.10.0 新增：
     · 歌单支持移除歌曲：列表每项右侧极简 × 删除键（桌面 hover 显现、移动端常显
       半透明），点击即从歌单移除；删除当前播放曲会自动续播下一首；
     · 歌单（含搜歌添加/移除的结果）持久化到 localStorage，刷新后保留；
     · 空歌单自动显示"歌单为空"并停止播放。
   已移除原组件中的"全域文本阅读器"(STORY ARCHIVE / MutationObserver 文本捕获)。
   所有 DOM 直接注入酒馆主页面，无 iframe 间接层。 */
(function(){
'use strict';
try{console.log('%c[Nico-Draw-Panel] v1.10.0 已加载：进度条·弹幕歌词·导航/快拍编辑·歌单移除','color:#818cf8;font-weight:bold');}catch(e){}
...
.nico-gallery-roll{display:flex;overflow-x:auto;scroll-snap-type:x mandatory;scrollbar-width:none;-webkit-overflow-scrolling:touch;transform:translateZ(0);}
...
.nico-gr-item img{width:100%;height:auto;display:block;}
...
.nico-drg{height:36px;width:100%;display:flex;justify-content:center;align-items:center;cursor:pointer;background:#FFF;border-top:1px solid #fafafa;flex-shrink:0;padding-bottom:env(safe-area-inset-bottom,0px);box-sizing:content-box;}
...
barEl.addEventListener('mousedown', function(e){ barDragging = true; seekAt(e); e.preventDefault(); });
document.addEventListener('mousemove', function(e){ if(barDragging) seekAt(e); });
document.addEventListener('mouseup', function(){ barDragging = false; });
barEl.addEventListener('touchstart', function(e){ barDragging = true; seekAt(e); }, {passive:true});
document.addEventListener('touchmove', function(e){ if(barDragging) seekAt(e); }, {passive:true});
