/* Qixian Draw Panel (Nico Draw Panel) - SillyTavern Extension v1.10.2
   ...
   v1.10.2 手机端布局修正：
     · 恢复面板占满整个手机界面（100dvh，不做高度缩短）；
     · 图片库图片显示高度上限 62vh（object-fit:contain 原比例完整显示），
       彻底避免上传高清超高竖图把内容撑爆、视觉上盖住底部收起横线；
     · 底部收起拖拽条 z-index 提升并固定在面板底部，无论添加多少图片都始终
       在视口内可见，且不被图片内容遮挡。
   已移除原组件中的"全域文本阅读器"(STORY ARCHIVE / MutationObserver 文本捕获)。
   所有 DOM 直接注入酒馆主页面，无 iframe 间接层。 */
(function(){
'use strict';
try{console.log('%c[Nico-Draw-Panel] v1.10.2 已加载：占满手机屏·图片限高·拖拽条常驻','color:#818cf8;font-weight:bold');}catch(e){}
